...
4. **関連する近年の学術成果**: このテーマに関連する学術的な知見や研究を**report_text本文内に**詳細に記述してください。
   - report_text内の学術セクションに、2つの学術成果それぞれについて**各300字以上の段落**を書いてください
   - 各段落に必ず含める内容: (a)著者名、(b)書名/論文名と出版年、(c)主要な主張・理論の具体的説明、(d)この記事テーマとの関連性
   - 「学術的根拠に基づいている」等の一般的な前置きだけで終わらせず、学術成果の中身を本文中で解説してください
   - related_research JSONフィールドにも同じ内容を要約して記載しますが、report_text本文が主体です
...
## 読みやすさのための構造指示（重要）:
- 各セクション（## 見出し）の冒頭1段落を「このセクションのポイント」として、太字で始めてください。例: **なぜ重要か:** この記事は...
- 以下の太字ラベルを適切な箇所で使用して、読者の視線のアンカーにしてください:
  - **なぜ重要か:** — セクション冒頭で核心を要約
  - **大局的に見ると:** — より広い文脈を示す段落の冒頭
  - **注目すべき点:** — 特に重要な発見・洞察を強調
  - **日本への示唆:** — 日本固有の文脈に言及する段落
- 重要な概念、固有名詞、数値は**太字**で強調してください
- 特に印象的な一文は > 引用ブロック（blockquote）として独立させてください。セクションごとに最大1つ
