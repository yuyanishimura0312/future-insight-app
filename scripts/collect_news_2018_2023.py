#!/usr/bin/env python3
"""
Fill the 2018-2023 gap in PESTLE news data.

Uses GDELT DOC API (covers 2017-02+).
Target: ~1000 articles per year across 6 PESTLE categories.

Strategy:
- Each year split into 4 quarters
- Each quarter × 6 categories = 24 requests/year
- 250 articles per request (GDELT max)
- Rate limit: 1 request per 5 seconds
- After collection, regenerate pestle_history.json and pestle_index.json
"""
...
def store_articles(articles: list[dict], category: str, year: int, quarter: int) -> int:
    """Store articles into DB, returns count of newly inserted."""
    conn = get_connection()
    month = (quarter - 1) * 3 + 1
...
def quarter_to_dates(year: int, quarter: int):
    """Convert (year, quarter) to GDELT date range strings."""
...
def regenerate_pestle_files():
    """Regenerate pestle_history.json, pestle_index.json, and latest.json from DB."""
    conn = get_connection()
    ...
    cats = ["Political", "Economic", "Social", "Technological", "Legal", "Environmental"]

    # pestle_history.json — top 20 articles per category per quarter
    history = {}
    ...
    # pestle_index.json — counts only (lightweight for initial load)
    index = {}
    ...
    # latest.json — top 20 per category overall
...
def main():
    # Target years: 2018-2023 (complete gap) + supplement 2017
    years_quarters = []
    for year in range(2017, 2024):
        for q in range(1, 5):
            # Skip 2017 Q1 (already partially collected)
            if year == 2017 and q < 2:
                continue
            years_quarters.append((year, q))

    total_requests = len(years_quarters) * len(PESTLE_QUERIES)
    est_minutes = total_requests * 5 / 60

    print(f"{'#' * 60}")
    print(f"  PESTLE News Gap Filler: 2017-Q2 ~ 2023-Q4")
    print(f"  Quarters: {len(years_quarters)}")
    ...
    for year, quarter in years_quarters:
        ...
        for category, config in PESTLE_QUERIES.items():
            ...
            print(f"  [{request_count}/{total_requests} {pct:.0f}%] {q_label} / {config['label_ja']}...",
                  end=" ", flush=True)
    ...
    # Regenerate all JSON files from DB
    ...
    """).fetchall()
    for y, c in rows:
        marker = " <<<" if int(y) >= 2017 and int(y) <= 2023 else ""
        print(f"    {y}: {c:,}{marker}")
    conn.close()
